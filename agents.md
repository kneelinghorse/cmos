...
## Project Overview
**Project Name**: CMOS Starter
**Project Type**: Mission orchestration starter kit
**Primary Language**: TypeScript (Node.js tooling) with Python helpers
**Framework**: Headless Node.js + Python utilities (no web framework)
...
- SQLite 3 database at `db/cmos.sqlite` is the canonical runtime store; keep file outputs as readable mirrors, not the source of authority.
- Regenerate baseline data with `python scripts/seed_sqlite.py --data-root <planning-workspace>` when you need to mirror an external backlog; omit `--data-root` to create an empty schema for fresh starters.
- Validate database ↔ file parity via `python scripts/validate_parity.py --data-root <planning-workspace>` prior to marking missions completed.
- File mirrors (`missions/backlog.yaml`, `PROJECT_CONTEXT.json`, `SESSIONS.jsonl`) ship empty by default; populate them from your planning workspace before seeding the database.
- Database health and synchronization metrics stream to `telemetry/events/database-health.jsonl`; inspect anomalies before progressing the backlog.
- Packaging pulls from the file mirrors after parity validation, ensuring downstream starters remain file-first while mirroring the SQLite truth.
- Lock documentation links with `python scripts/validate_foundational_refs.py`; required before closing documentation or agent refresh missions.
...
```
project-root/
├── agents.md                # Canonical instructions for AI collaborators
├── context/                 # Shared SQLite clients, mission runtime helpers, validation runners
├── docs/                    # Shipping documentation bundled with the starter
├── foundational-docs/       # Roadmap & technical architecture templates for downstream teams
├── missions/                # Mission templates and sprint backlog source of truth
├── runtime/                 # Orchestration assets (RSIP, boomerang checkpoints, workers)
├── scripts/                 # Automation (packaging, parity validation, SQLite seeding)
├── db/                      # SQLite schema, seeded database (`cmos.sqlite`), migration tooling
├── telemetry/               # Event schemas + health signals (mirrors DB telemetry)
├── templates/               # Mission Protocol domain packs
├── tests/                   # Guardrail fixtures across integration/security/quality/perf
└── dist/                    # Starter build output
```

> **Planning workspace**: Sprint planning artefacts, historical sessions, and research notes now live outside this starter bundle (e.g. `/Users/systemsystems/portfolio/cmos-current-dev-pm/`). Point tooling at that directory with `--data-root` when you need the full backlog.

### Documentation Boundaries
- `docs/` ships the active roadmap, technical architecture, packaging, and SQLite guides that evolve with each sprint.
- `foundational-docs/` stores the canonical roadmap (`foundational-docs/roadmap_template.md`) and architecture (`foundational-docs/tech_arch_template.md`) templates that downstream teams clone before making project-specific edits.
- The project-management workspace (external to this repo) retains maintainer-only playbooks and historical research; do not reference it in shipping guidance.
...
NODE_ENV=production
DEBUG=false
DB_PATH=/var/app/data/cmos.sqlite
```
...
sqlite:
  ...
  seed_script: scripts/seed_sqlite.py
  parity_check: scripts/validate_parity.py
telemetry:
  database_health_stream: telemetry/events/database-health.jsonl
```
